        ...
        for (let pair of this.qtyPricePairs) {
          if (this.currentQtyForVolumePricing >= pair[0]) {
            ...
            if (pricePerItemCurrent) {
              this.classList.contains('variant-item__price-per-item') ? pricePerItemCurrent.innerHTML = (window.quickOrderListStrings?.each || '[money]').replace('[money]', pair[1]) : pricePerItemCurrent.innerHTML = pair[1];
            }
            break;
          }
        }
