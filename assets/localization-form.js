    ...
    class LocalizationForm extends HTMLElement {
      ...
      filterCountries() {
        ...
        allCountries.forEach(item => {
          ...
          const countryName = this.normalizeString(countryEl ? countryEl.textContent : "");
          if (countryName.indexOf(searchValue) > -1) {
            if (item.parentElement) item.parentElement.classList.remove("hidden");
            visibleCountries++;
          } else {
            if (item.parentElement) item.parentElement.classList.add("hidden");
            visibleCountries--;
          }
        });
        ...
        if (this.elements.liveRegion) {
          ...
          ).replace("[count]", visibleCountries);
        }

        const countrySelector = this.querySelector(".country-selector");
        if (countrySelector) countrySelector.scrollTop = 0;
        const countrySelectorList = this.querySelector(".country-selector__list");
        if (countrySelectorList) countrySelectorList.scrollTop = 0;
      }
