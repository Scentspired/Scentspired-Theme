...
function fetchUrl(urlPath, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const reqOptions = {
      ...
      headers: {
        'User-Agent': 'Mozilla/5.0 (Scentspired Theme Guardian automated smoke tester; +https://scentspired.com)',
        ...(options.headers || {})
      },
      timeout: 10000
    };
...
async function runSmokeTests() {
  ...
  try {
    ...
    assert(res.statusCode >= 200 && res.statusCode < 400, '1.1: Homepage returns HTTP 200 OK', `Status: ${res.statusCode}`);
    assert(res.body.includes('Scentspired') || res.body.includes('scentspired'), '1.2: Page body contains store brand identifier');
  } catch (e) {
  ...
  try {
    ...
    assert(pdpRes.body.includes('product-form') || pdpRes.body.includes('ProductForm') || pdpRes.body.includes('AddToCart') || pdpRes.body.includes('add-to-cart'), '2.2: Product detail page contains active Add-to-Cart form container');
  ...
  try {
    ...
    let cartJson = null;
    try { cartJson = JSON.parse(cartRes.body); } catch (e) {}
    assert(cartJson !== null, '3.2: /cart.js returns valid JSON schema');
    assert(cartJson && typeof cartJson.item_count === 'number', '3.3: cart.item_count field present and numeric');
  ...
  try {
    ...
    assert(chkRes.statusCode >= 200 && chkRes.statusCode < 400, '4.1: /checkout entry point responds without server crash', `Status: ${chkRes.statusCode}`);
  } catch (e) {
  ...
  // Summary
  console.log('\n┌──────────────────────────────────────────────────────────────┐');
  console.log(`│  Checks Run: ${String(passed + failed).padStart(2)}      │  Passed: ${String(passed).padStart(2)}      │  Failed: ${String(failed).padStart(2)}          │`);
  console.log('└──────────────────────────────────────────────────────────────┘');
