...
**Generated At:** `2026-08-30 11:24:12 UTC`  
**Branch:** `develop`  
...
| **Total Lines Inspected**     | `68,323` lines           | 🔍 Complete            |
| **Active Rules Evaluated**    | `12 Rules`               | 🛡️ Active              |
| **Errors (Blockers)**         | `2` errors               | ❌ **Blocking Deploy** |
...
| :-------------------------------------------------- | :------- | :-------------------------- | :----------------------------------------------------------------------------------------------- |
| [Line 1](#snippets-cart-drawer-liquid-line-1)       | 🔴 ERROR | `balanced-script-tags`      | Unbalanced script tags: 1 <script> vs 0 </script> — missing </script>                            |
| [Line 2320](#snippets-cart-drawer-liquid-line-2320) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'plusBtn' from DOM lookup used without null check — will crash if element doesn't exist |

<details open>
<summary><b>🔍 View Code Snippets for <code>snippets/cart-drawer.liquid</code> (2 items)</b></summary>

<a id="snippets-cart-drawer-liquid-line-1"></a>

#### 📍 Line 1 — `balanced-script-tags` (🔴 ERROR)

> **Technical Finding:** Unbalanced script tags: 1 <script> vs 0 </script> — missing </script>

```liquid
>> 1 |
<style>
       2 |   .sp-bundle-container {
       3 |     background-color: #f3f3f3;
```

<a id="snippets-cart-drawer-liquid-line-2320"></a>

#### 📍 Line 2320 — `no-unguarded-dom-variable` (🔴 ERROR)

> **Technical Finding:** Variable 'plusBtn' from DOM lookup used without null check — will crash if element doesn't exist

```liquid
2318 | // Disable + button if at max inventory using CSS class 2319 | if (cartItem &&
cartItem.quantity >= inventory && plusBtn) { >> 2320 | plusBtn.classList.add('sp-qty-disabled');
2321 | plusBtn.disabled = true; 2322 | } else if (plusBtn) {
```
