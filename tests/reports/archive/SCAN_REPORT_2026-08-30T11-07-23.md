...
**Generated At:** `2026-08-30 11:07:23 UTC`  
**Branch:** `develop`  
...
| **Total Lines Inspected** | `67,650` lines | 🔍 Complete |
| **Active Rules Evaluated** | `12 Rules` | 🛡️ Active |
| **Errors (Blockers)** | `5` errors | ❌ **Blocking Deploy** |
...
### 📄 `assets/facets.js` (4 findings)

| Line | Severity | Rule | Defect Summary |
| :--- | :--- | :--- | :--- |
| [Line 109](#assets-facets-js-line-109) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'pgc' from DOM lookup used without null check — will crash if element doesn't exist |
| [Line 245](#assets-facets-js-line-245) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'targetSummary' from DOM lookup used without null check — will crash if element doesn't exist |
| [Line 252](#assets-facets-js-line-252) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'targetHeaderElement' from DOM lookup used without null check — will crash if element doesn't exist |
| [Line 276](#assets-facets-js-line-276) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'targetFacetsList' from DOM lookup used without null check — will crash if element doesn't exist |

<details open>
<summary><b>🔍 View Code Snippets for <code>assets/facets.js</code> (4 items)</b></summary>

<a id="assets-facets-js-line-109"></a>
#### 📍 Line 109 — `no-unguarded-dom-variable` (🔴 ERROR)
> **Technical Finding:** Variable 'pgc' from DOM lookup used without null check — will crash if element doesn't exist

```javascript
     107 |     const newPgc = parsed.getElementById('ProductGridContainer');
     108 |     if (newPgc) {
>>   109 |       pgc.innerHTML = newPgc.innerHTML;
     110 |     }
     111 | 
```

<a id="assets-facets-js-line-245"></a>
#### 📍 Line 245 — `no-unguarded-dom-variable` (🔴 ERROR)
> **Technical Finding:** Variable 'targetSummary' from DOM lookup used without null check — will crash if element doesn't exist

```javascript
     243 | 
     244 |     if (sourceSummary && targetSummary) {
>>   245 |       targetSummary.outerHTML = sourceSummary.outerHTML;
     246 |     }
     247 | 
```

<a id="assets-facets-js-line-252"></a>
#### 📍 Line 252 — `no-unguarded-dom-variable` (🔴 ERROR)
...
```javascript
     250 | 
     251 |     if (sourceHeaderElement && targetHeaderElement) {
>>   252 |       targetHeaderElement.outerHTML = sourceHeaderElement.outerHTML;
     253 |     }
     254 | 
```

<a id="assets-facets-js-line-276"></a>
#### 📍 Line 276 — `no-unguarded-dom-variable` (🔴 ERROR)
> **Technical Finding:** Variable 'targetFacetsList' from DOM lookup used without null check — will crash if element doesn't exist

```javascript
     274 | 
     275 |     if (sourceFacetsList && targetFacetsList) {
>>   276 |       targetFacetsList.outerHTML = sourceFacetsList.outerHTML;
     277 |     }
     278 |   }
```
...
### 📄 `assets/product-info.js` (1 findings)

| Line | Severity | Rule | Defect Summary |
| :--- | :--- | :--- | :--- |
| [Line 379](#assets-product-info-js-line-379) | 🔴 ERROR | `no-unguarded-dom-variable` | Variable 'updated' from DOM lookup used without null check — will crash if element doesn't exist |

<details open>
<summary><b>🔍 View Code Snippets for <code>assets/product-info.js</code> (1 items)</b></summary>

<a id="assets-product-info-js-line-379"></a>
#### 📍 Line 379 — `no-unguarded-dom-variable` (🔴 ERROR)
> **Technical Finding:** Variable 'updated' from DOM lookup used without null check — will crash if element doesn't exist

```javascript
     377 |             const attributes = ['data-cart-quantity', 'data-min', 'data-max', 'step'];
     378 |             for (let attribute of attributes) {
>>   379 |               const valueUpdated = updated.getAttribute(attribute);
     380 |               if (valueUpdated !== null) {
     381 |                 current.setAttribute(attribute, valueUpdated);
```
