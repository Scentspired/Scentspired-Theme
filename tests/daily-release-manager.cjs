...
if (!fs.existsSync(QUEUE_FILE)) {
  console.error('[FATAL] remediation-queue.json not found. Run tests/build-remediation-queue.cjs first.');
  process.exit(1);
}
...
if (isListQueue) {
  ...
  const daysDone = new Set((queueData.queue || []).filter(i => i.status === 'released' && i.type === 'warning').map(i => i.day)).size;

  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║  SCENTSPIRED THEME GUARDIAN — 60-Day Warning Roadmap         ║');
  console.log('╚══════════════════════════════════════════════════════════════╝');
  console.log(`\n📊 Status Summary:`);
  console.log(`   • Core Errors:     100% RELEASED (${queueData.totalErrors || 291}/${queueData.totalErrors || 291} Resolved)`);
  ...
  for (let d = daysDone + 1; d <= Math.min(daysDone + 5, totalDays); d++) {
    ...
    if (items.length > 0) {
      console.log(`   [Day ${String(d).padStart(2)}/${totalDays}] ${items.length} items: ${items.map(i => i.id).join(', ')} (${items[0].phase || 'Optimization'})`);
    }
  }
  console.log('\n');
  process.exit(0);
...
const batchItems = queueData.queue.filter(item => item.day === targetDay && item.status === 'pending');
...
console.log(`║  Processing Day ${String(targetDay).padStart(2)} of 60 (${batchItems.length} warning items)              ║`);
console.log('╚══════════════════════════════════════════════════════════════╝\n');
