...
assert(sanitizedXss.length > 0 && typeof sanitizedXss === 'string', 'AE1.1: XSS script tag string processed safely without executing or crashing', 'SUITE AE');

const massivePayload = 'A'.repeat(5000);
const sanitizedMassive = sanitizeSearchQuery(massivePayload);
assert(sanitizedMassive.length === 200, 'AE1.2: 5,000-character flood string cleanly truncated to 200 character boundary', 'SUITE AE');

const unicodePayload = '🌸✨ Scent & Spîrèd ⚜️ 100% Parfum \u0000 (Black Opium)';
const sanitizedUnicode = sanitizeSearchQuery(unicodePayload);
assert(!sanitizedUnicode.includes('\u0000') && sanitizedUnicode.includes('🌸✨'), 'AE1.3: Null byte safely stripped while preserving valid international emojis and accents', 'SUITE AE');

assert(sanitizeSearchQuery(null) === '' && sanitizeSearchQuery(undefined) === '', 'AE1.4: Null and undefined queries safely return empty string', 'SUITE AE');
...
async function runCartStressSimulation() {
  ...
  assert(cart.quantity >= 10, 'AF1.1: Cart quantity never regressed below initial baseline under 50-request flood', 'SUITE AF');
  assert(cart.successfulUpdates + cart.droppedRequests === 50, 'AF1.2: All 50 concurrent requests accounted for (either processed or blocked by lock)', 'SUITE AF');
  assert(cart.lock === false, 'AF1.3: Concurrency lock guaranteed to be released after flood resolves', 'SUITE AF');
}
...
assert(didThrow === false, 'AG1.1: Safari Private Mode SecurityError caught cleanly without throwing unhandled exception', 'SUITE AG');
assert(brokenStorage.getItem('scentspired_cart_backup') !== null, 'AG1.2: Fallback storage successfully persisted data in-memory', 'SUITE AG');
...
async function runFlakyNetworkTests() {
  ...
  const slowPromise = new Promise(r => setTimeout(() => r({ ok: true, text: async () => '{"ok":true}' }), 200));
  const resTimeout = await handleFlakyResponse(slowPromise, 20);
  assert(resTimeout.error === 'REQUEST_TIMEOUT', 'AH1.1: Slow network response (200ms) aborted cleanly at 20ms threshold', 'SUITE AH');
  ...
  assert(resEmpty.error === 'EMPTY_RESPONSE', 'AH1.2: Empty HTTP 200 body handled cleanly without JSON parse exception', 'SUITE AH');

  // Test 3: Corrupt truncated JSON
  const corruptPromise = Promise.resolve({ ok: true, text: async () => '{"status": "ok", "items": [' });
  const resCorrupt = await handleFlakyResponse(corruptPromise);
  assert(resCorrupt.error === 'MALFORMED_JSON', 'AH1.3: Truncated JSON payload caught safely without unhandled SyntaxError', 'SUITE AH');
}
...
layout.openDrawer();
assert(layout.isDrawerOpen && layout.bodyLocked, 'AI1.1: Opening drawer locks body scroll', 'SUITE AI');

// Simulate rapid resize: Desktop -> Mobile -> Tablet -> Desktop
layout.resize(375);
layout.resize(768);
layout.resize(1440);
assert(layout.isDrawerOpen && layout.bodyLocked, 'AI1.2: Drawer state and scroll lock preserved across multiple rapid viewport resizes', 'SUITE AI');

layout.closeDrawer();
assert(!layout.isDrawerOpen && !layout.bodyLocked, 'AI1.3: Closing drawer safely unlocks body scroll', 'SUITE AI');
...
assert(tab2CartCount === 4, 'AJ1.1: Tab 2 synchronized cart item count (4 items) from broadcast', 'SUITE AJ');
assert(tab3CartCount === 4, 'AJ1.2: Tab 3 synchronized cart item count (4 items) from broadcast', 'SUITE AJ');
