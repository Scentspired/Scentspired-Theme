...
global.customElements = {
  ...
  define: function(name, cls) { this.registry[name] = cls; }
};
...
async function runSuiteA() {
  ...
  // Test A1: Standard Add to Cart Flow with Cart Drawer Synchronization
  {
    ...
    window.updateDossierCartUI = (cart) => { drawerUpdated = true; };
    window.openDossierCart = () => { drawerOpened = true; };
    ...
    global.fetch = async (url) => {
      ...
      if (url === '/cart.js') {
        return { json: async () => ({ item_count: 1, items: [{ id: 44556677, title: 'Smoky Leather' }] }) };
      }
      return { json: async () => ({}) };
    };
  ...
  // Test A2: Multi-Tap / Rage-Click Debouncing
  {
    ...
    await new Promise(r => setTimeout(r, 200));
    assert(dispatchCount === 1, 'A2.1: Exactly 1 request dispatched despite 5 rapid clicks', `Count: ${dispatchCount}`);
  }
  ...
  {
    global.fetch = async () => { throw new Error('500 Internal Server Error'); };
...
async function runSuiteC() {
  ...
  // Test C1: Apostrophe in Brand Name (e.g. Victoria's Secret) Evaluates Safely
  {
    ...
    function selectBrand(tag) { selectedBrand = tag; }
    ...
    assert(brandHTML.includes("selectBrand('victoria\\'s secret')"), 'C1.1: Brand tag with apostrophe properly escaped in inline onclick');
    assert(brandHTML.includes("selectBrand('kilian\\'s angels')"), 'C1.2: Second brand tag with apostrophe properly escaped');
  }
...
async function runSuiteD() {
  ...
  // Test D2: openDossierCart Adds Active Class
  {
    ...
    openDossierCart();
    assert(drawer.classList.contains('is-open'), 'D2.1: Cart drawer successfully opened with .is-open class');
  }
}
...
async function runSuiteF() {
  ...
  // Test F1: Missing Variant Input Handled Safely in Add to Cart
  {
    ...
    form.dispatchEvent('submit');
    assert(submittedWithoutId === true, 'F1.1: Missing variant ID cleanly blocked before API dispatch');
...
async function runSuiteG() {
  ...
  // Test G1: Server returns Cloudflare HTML error page instead of JSON
  {
    ...
    assert(handledGracefully === true, 'G1.1: 502 HTML error caught in .catch() without crashing UI');
...
async function runSuiteH() {
  ...
  // Test H1: formatPrice with undefined, null, or corrupted inputs
  {
    ...
    assert(formatPrice(undefined) === '$0.00', 'H1.3: undefined input defaults to $0.00 (never $NaN)');
...
async function runSuiteI() {
  ...
  // Test I1: Shipping progress percentage clamps safely between 0% and 100%
  {
    ...
    assert(empty.percentage === 0 && empty.remaining === 50 && !empty.isUnlocked, 'I1.1: $0 cart shows 0% progress and $50 remaining');

    const partial = calculateShippingProgress(3500);
    assert(partial.percentage === 70 && partial.remaining === 15 && !partial.isUnlocked, 'I1.2: $35 cart shows 70% progress and $15 remaining');

    const unlocked = calculateShippingProgress(7500);
    assert(unlocked.percentage === 100 && unlocked.remaining === 0 && unlocked.isUnlocked, 'I1.3: $75 cart clamps to 100% and unlocks free shipping');
  }
}
...
async function runSuiteJ() {
  ...
  // Test J1: proceedToDossierCheckout redirects cleanly to /checkout
  {
    ...
    assert(emptyResult === false && redirectedUrl === null, 'J1.1: Empty cart checkout blocked from firing dead redirect');

    const validResult = proceedToDossierCheckout(2);
    assert(validResult === true && redirectedUrl === '/checkout', 'J1.2: Valid cart immediately redirects to /checkout');
  }
}
...
async function runSuiteK() {
  ...
  // Test K1: Registration Form Input Validation
  {
    function validateCustomerRegistration(fields) {
      const errors = {};
      if (!fields.firstName || fields.firstName.trim() === '') errors.firstName = 'First name required';
    ...
    assert(!invalidSubmission.isValid && invalidSubmission.errors.firstName && invalidSubmission.errors.email && invalidSubmission.errors.password, 'K1.1: Invalid registration inputs caught before network dispatch');
    ...
    assert(validSubmission.isValid && Object.keys(validSubmission.errors).length === 0, 'K1.2: Valid registration inputs pass validation cleanly');
  }
  ...
  // Test K2: Mobile & Desktop Account Navigation Routing
  {
    ...
    assert(getAccountDestination(false, null) === '/account/login', 'K2.1: Guest shopper correctly routed to /account/login');
    assert(getAccountDestination(true, null) === '/account', 'K2.2: Logged-in customer correctly routed to /account dashboard');
    assert(getAccountDestination(false, '/account/register') === '/account/register', 'K2.3: Custom register link preserved');
  }
}
...
async function runSuiteL() {
  ...
  // Test L1: 5-Box Bundle Properties
  {
    ...
    assert(Object.keys(properties).length === 15, 'L1.1: Exactly 15 line item properties generated for 5-box set');
    assert(properties['Product 1'] === 'Smoky Leather' && properties['SKU 1'] === 'SL-50' && properties['Variant ID 1'] === 101, 'L1.2: Slot 1 metadata accurate');
    assert(properties['Product 5'] === 'Golden Amber' && properties['SKU 5'] === 'GA-50' && properties['Variant ID 5'] === 105, 'L1.3: Slot 5 metadata accurate');
  }
  ...
  // Test L2: Partial/Null slot defense (Customer cannot add incomplete bundle)
  {
    ...
    assert(validateBundleComplete([{}, {}, null, {}, {}], 5) === false, 'L2.1: Bundle with 4/5 items blocked');
    assert(validateBundleComplete([{}, {}, {}, {}, {}], 5) === true, 'L2.2: Complete 5/5 bundle verified valid');
  }
}
...
async function runSuiteM() {
  ...
  // Test M1: Telemetry payload generation and breadcrumb trail
  {
    ...
    assert(payload.type === 'UNHANDLED_EXCEPTION' && payload.breadcrumbs.length === 2, 'M1.1: Error payload formatted with breadcrumb trail');
    assert(payload.breadcrumbs[1].message === 'Clicked Add to Cart button', 'M1.2: Breadcrumb order and metadata preserved');
  }
  ...
  // Test M2: LocalStorage Persistent Crash Vault
  {
    ...
    function saveToVault(payload) {
      const existing = mockStorage['scentspired_telemetry_vault'] ? JSON.parse(mockStorage['scentspired_telemetry_vault']) : [];
    ...
    assert(vault.length === 1 && vault[0].type === 'COMMERCE_NETWORK_ERROR', 'M2.1: Crash event successfully persisted in LocalStorage vault');
  }
}
...
async function runSuiteN() {
  ...
  // Test N1: Regex special character escaping in search terms
  {
    ...
    assert(allEscapedSafely, 'N1.1: Special characters in search query safely escaped without RegExp crash');
  }
  ...
  // Test N2: Empty search query does not fire unnecessary network request
  {
    ...
    function queryPredictiveSearch(searchTerm) {
      ...
      fetchCount++;
      return Promise.resolve({ resources: { results: { products: [{ id: 1, title: 'Sample' }] } } });
    }

    await queryPredictiveSearch('');
    await queryPredictiveSearch('   ');
    await queryPredictiveSearch('a');
    assert(fetchCount === 0, 'N2.1: Search queries < 2 chars short-circuit before network dispatch');
  ...
  // Test N3: Predictive Search dropdown rendering with null-safe results container
  {
    ...
    function renderResults(results, el) {
      ...
      el.innerHTML = results.map(r => `<div class="search-item" data-id="${r.id}">${r.title}</div>`).join('');
      return true;
    }
    ...
    renderResults([{ id: 101, title: 'Smoky Leather' }, { id: 102, title: 'Amber Floral' }], container);
    assert(container.innerHTML.includes('Smoky Leather') && container.innerHTML.includes('data-id="101"'), 'N3.2: Search items render into container with valid markup');
  }
}
...
async function runSuiteO() {
  ...
  // Test O1: Mobile Menu Manager Open/Close state transitions
  {
    ...
    manager.open();
    assert(manager.isOpen === true && menuDrawer.classList.contains('is-active'), 'O1.1: Mobile menu opens and applies is-active class');
    assert(trigger.getAttribute('aria-expanded') === 'true', 'O1.2: Trigger aria-expanded updated to true on open');

    manager.close();
    assert(manager.isOpen === false && !menuDrawer.classList.contains('is-active'), 'O1.3: Mobile menu closes and cleans up is-active class');
    assert(trigger.getAttribute('aria-expanded') === 'false', 'O1.4: Trigger aria-expanded updated to false on close');
  }
  ...
  // Test O2: Backward compatibility alias safety
  {
    ...
    assert(mockGlobal.MobileNavigation === mockGlobal.MobileMenuManager, 'O2.1: MobileNavigation legacy alias points to MobileMenuManager');
    const instance = new mockGlobal.MobileNavigation();
    assert(instance instanceof MobileMenuManager, 'O2.2: Instantiating MobileNavigation yields MobileMenuManager instance');
  }
}
...
async function runSuiteP() {
  ...
  // Test P1: Format money helper across global currencies
  {
    ...
    assert(formatMoney(12550, '£{{amount}}') === '£125.50', 'P1.2: GBP price 12550 cents formats to £125.50');
    assert(formatMoney(0) === '$0.00', 'P1.3: 0 cents formats to $0.00');
    assert(formatMoney('invalid') === '$0.00', 'P1.4: Non-numeric currency input safely falls back to $0.00');
  }
  ...
  // Test P2: Localization form hidden input sync
  {
    ...
    selectCountry('CA');
    assert(countryCodeInput.value === 'CA', 'P2.1: Country code updated to CA in localization form');
  }
}
...
async function runSuiteQ() {
  ...
  // Test Q1: Filter third-party iframe and tracking script errors
  {
    ...
    assert(shouldDropError({ filename: 'https://cdn.linktr.ee/main.js', message: 'Failed to fetch' }) === true, 'Q1.1: Linktree 3rd party script error correctly dropped');
    assert(shouldDropError({ filename: 'https://fwcdn3.com/embed.js', message: 'NetworkError' }) === true, 'Q1.2: Firework video script error correctly dropped');
    assert(shouldDropError({ filename: 'https://scentspired.com/theme.js', message: 'Script error.' }) === true, 'Q1.3: Generic CORS "Script error." correctly dropped');
    assert(shouldDropError({ filename: 'https://scentspired.com/assets/product-form.js', message: 'TypeError: Cannot read properties of undefined' }) === false, 'Q1.4: First-party theme JS error correctly PRESERVED for capture');
  }
}
...
async function runSuiteR() {
  ...
  // Test R1: Verify no duplicate variable declaration collisions in bundle scopes
  {
    ...
    const fiveBox = calculateBundleTotal(5, 29.00, 15.00);
    assert(fiveBox.subtotal === '145.00' && fiveBox.total === '130.00', 'R1.1: 5-Box set computes $145.00 subtotal - $15.00 discount = $130.00 total');

    const trioSet = calculateBundleTotal(3, 29.00, 10.00);
    assert(trioSet.subtotal === '87.00' && trioSet.total === '77.00', 'R1.2: Trio set computes $87.00 subtotal - $10.00 discount = $77.00 total');
  }
  ...
  // Test R2: Verify bundle slot validation (cannot checkout with incomplete slots)
  {
    ...
    assert(partial5.isValid === false && partial5.missingCount === 2, 'R2.1: Incomplete 5-box (3/5 filled) is blocked with missingCount=2');

    const complete5 = validateBundleSlots(5, [{ id: 'v1' }, { id: 'v2' }, { id: 'v3' }, { id: 'v4' }, { id: 'v5' }]);
    assert(complete5.isValid === true && complete5.missingCount === 0, 'R2.2: Complete 5-box (5/5 filled) passes validation cleanly');
  }
}
...
async function runSuiteS() {
  ...
  // Test S1: Quantity modification (increment, decrement, zero-removal)
  {
    ...
    assert(cart.find(i => i.id === 'item_1').quantity === 3, 'S1.1: Item quantity incremented from 2 to 3');

    updateItemQuantity('item_2', 0);
    assert(cart.length === 1 && !cart.find(i => i.id === 'item_2'), 'S1.2: Setting quantity to 0 removes line item from cart');
  }
  ...
  // Test S2: Total item count and subtotal recalculation
  {
    ...
    function calculateCartSummary(items) {
      ...
      return { totalItems, subtotalCents, subtotalFormatted: `$${(subtotalCents / 100).toFixed(2)}` };
    }

    const summary = calculateCartSummary(cart);
    assert(summary.totalItems === 5, 'S2.1: Total cart item count calculated accurately (3 + 2 = 5 items)');
    assert(summary.subtotalFormatted === '$117.00', 'S2.2: Cart subtotal formatted correctly ((3*29) + (2*15) = $117.00)');
  }
}
...
async function runSuiteT() {
  ...
  // Test T1: FIFO Eviction prevents memory and LocalStorage exhaustion
  {
    ...
    assert(entries[0].id === 16 && entries[9].id === 25, 'T1.2: Oldest 15 entries evicted FIFO, preserving latest 16-25');
  }
  ...
  // Test T2: Telemetry JSON stringify safety with cyclic references
  {
    ...
    assert(serialized !== null && serialized.includes('[Circular Reference]'), 'T2.1: Cyclic payload safely stringified without throwing TypeError');
  }
}
...
async function runAllSuites() {
  ...
  console.log('\n┌──────────────────────────────────────────────────────────────┐');
  console.log(`│  Total Assertions: ${String(totalPassed + totalFailed).padStart(2)}  │  Passed: ${String(totalPassed).padStart(2)}  │  Failed: ${String(totalFailed).padStart(2)}          │`);
  console.log('└──────────────────────────────────────────────────────────────┘');
