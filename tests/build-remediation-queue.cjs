...
try {
  ...
  if (e.stdout) {
    try { scanResult = JSON.parse(e.stdout); } catch (err) {}
  }
}
...
// 3. Asset scripts
const priorityFiles = [
  'five-box.liquid', 'trio-set.liquid', 'bundle.liquid', 'discovery.liquid',
  'best-sellers.liquid', 'bundlediscovery.liquid',
  'header.liquid', 'product-info-tab.liquid', 'cart-drawer.liquid', 'theme.liquid'
];
...
warnings.forEach((warn, idx) => {
  ...
  queue.push({
    ...
    status: 'pending',
    releasedAt: null,
    phase: warnDay <= 30 ? 'Month 1: Accessibility & DOM Efficiency' : 'Month 2: Codebase Hygiene & Cleanup'
  });
});
...
console.log(`   → Non-Blocking Warnings: ${warnings.length} Spanned Across ${TOTAL_WARNING_DAYS} Days (2 Months)`);
