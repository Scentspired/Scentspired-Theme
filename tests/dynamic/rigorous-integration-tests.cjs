...
assert(
  checkInventoryAvailability(vInStock, 1, 0) === true,
  "U1.1: In-stock variant with 5 units allows adding 1 unit",
  "SUITE U"
);
assert(
  checkInventoryAvailability(vInStock, 3, 3) === false,
  "U1.2: Adding 3 units when 3 already in cart exceeds stock limit of 5",
  "SUITE U"
);
assert(
  checkInventoryAvailability(vSoldOut, 1, 0) === false,
  "U1.3: Sold out variant (available=false) correctly blocked",
  "SUITE U"
);
assert(
  checkInventoryAvailability(vContinuePolicy, 10, 0) === true,
  "U1.4: Pre-order variant (policy=continue) allows purchasing past 0 stock",
  "SUITE U"
);
assert(
  checkInventoryAvailability(null, 1, 0) === false,
  "U1.5: Null/undefined variant safely rejected without TypeError",
  "SUITE U"
);
...
assert(
  queue.enqueue(401, "5") === true,
  "V1.1: Valid integer quantity string enqueued cleanly",
  "SUITE V"
);
assert(queue.enqueue(402, -3) === false, "V1.2: Negative quantity rejected at boundary", "SUITE V");
assert(
  queue.enqueue(403, "invalid") === false,
  "V1.3: Non-numeric quantity string rejected",
  "SUITE V"
);
queue.enqueue(404, 2);
const batch = queue.processBatch();
assert(
  batch[401] === 5 && batch[404] === 2,
  "V2.1: Multi-item batch updates dictionary created accurately",
  "SUITE V"
);
...
assert(
  standardResult.unitPrice === 3000 &&
    standardResult.subtotal === 6000 &&
    standardResult.savings === 0,
  "W1.1: Standard pricing applies for 2 units below tier 1 (2 * $30.00 = $60.00)",
  "SUITE W"
);

const tier1Result = calculateVolumePrice(3000, 3, volumeTiers);
assert(
  tier1Result.unitPrice === 2500 && tier1Result.subtotal === 7500 && tier1Result.savings === 1500,
  "W1.2: Tier 1 discount applies for 3 units ($25.00/ea, $15.00 total savings)",
  "SUITE W"
);

const tier2Result = calculateVolumePrice(3000, 6, volumeTiers);
assert(
  tier2Result.unitPrice === 2000 && tier2Result.subtotal === 12000 && tier2Result.savings === 6000,
  "W1.3: Tier 2 discount applies for 6 units ($20.00/ea, $60.00 total savings)",
  "SUITE W"
);
...
assert(
  gallery.setActiveMedia(1) === true && gallery.activeIndex === 1,
  "X1.1: Media switch to index 1 updates active slide",
  "SUITE X"
);
assert(
  gallery.setActiveMedia(10) === false && gallery.activeIndex === 1,
  "X1.2: Out-of-bounds media index rejected safely",
  "SUITE X"
);
assert(
  gallery.openModal() === "img2.jpg" && gallery.isModalOpen === true,
  "X1.3: Modal opens with current active media item",
  "SUITE X"
);
gallery.closeModal();
...
assert(
  queryString.includes("filter.v.price.gte=20.00"),
  "Y1.1: Price floor filter encoded correctly",
  "SUITE Y"
);
assert(
  queryString.includes("filter.p.tag=woody") && queryString.includes("filter.p.tag=floral"),
  "Y1.2: Multi-value array filters serialized into separate URL parameters",
  "SUITE Y"
);
assert(
  !queryString.includes("empty_filter"),
  "Y1.3: Empty filter string values cleanly stripped from query",
  "SUITE Y"
);
assert(
...
assert(
  formatCurrency(1499, "GBP") === "£14.99",
  "AA1.1: 1499 cents correctly formats to £14.99",
  "SUITE AA"
);
assert(
  formatCurrency(2900, "EUR") === "€29.00",
  "AA1.2: 2900 cents correctly formats to €29.00",
  "SUITE AA"
);
assert(
  formatCurrency(12550, "CAD") === "CA$125.50",
  "AA1.3: 12550 cents correctly formats to CA$125.50",
  "SUITE AA"
);
assert(
  formatCurrency("invalid", "GBP") === "£0.00",
  "AA1.4: Invalid string input safely falls back to £0.00 without throwing",
  "SUITE AA"
);
assert(
  formatCurrency(0, "USD") === "$0.00",
  "AA1.5: 0 cents formats correctly to $0.00",
  "SUITE AA"
);
...
async function runConcurrencyTest() {
  ...
  assert(
    successCount === 1,
    "AB1.1: Exactly 1 dispatch permitted during rapid concurrent clicks",
    "SUITE AB"
  );
  assert(
    lockedCount === 2,
    "AB1.2: 2 duplicate rapid clicks successfully blocked at gate",
    "SUITE AB"
  );
  assert(
    atc.isProcessing === false,
    "AB1.3: Lock safely released after request completion",
    "SUITE AB"
  );
}
  ...
  try {
    if (scenario === "422_SOLD_OUT") {
      const response = {
        ok: false,
        status: 422,
        json: async () => ({ description: "Sold out", message: "Item is sold out" }),
      };
      if (!response.ok) {
...
async function runNetworkRecoveryTests() {
  ...
  assert(
    res422.errorMessage === "Sold out" && res422.isButtonUnlocked,
    "AC1.1: Shopify 422 Sold Out caught and button unlocked in finally",
    "SUITE AC"
  );

  const res502 = await simulateNetworkFetch("502_CLOUDFLARE_HTML");
  assert(
    res502.errorMessage.includes("502") && res502.isButtonUnlocked,
    "AC1.2: Cloudflare 502 HTML response caught cleanly without JSON parse crash",
    "SUITE AC"
  );

  const resOffline = await simulateNetworkFetch("OFFLINE_DROPOUT");
  assert(
    resOffline.errorMessage === "Failed to fetch" && resOffline.isButtonUnlocked,
    "AC1.3: Network offline dropout caught safely and UI unlocked",
    "SUITE AC"
  );
}
...
function calculateCartTierDiscount(subtotalCents) {
  ...
  if (subtotalCents >= 10000) {
    discountCents = Math.round(subtotalCents * 0.2); // 20% off
    nextTierThreshold = 0;
  } else if (subtotalCents >= 6000) {
    discountCents = Math.round(subtotalCents * 0.1); // 10% off
...
assert(
  tierLow.discount === 0 && tierLow.nextTierThreshold === 1500,
  "AD1.1: $45.00 subtotal receives $0 discount and calculates $15.00 to Tier 1",
  "SUITE AD"
);

const tierMid = calculateCartTierDiscount(8000);
assert(
  tierMid.discount === 800 && tierMid.total === 7200 && tierMid.nextTierThreshold === 2000,
  "AD1.2: $80.00 subtotal receives 10% ($8.00) discount, total = $72.00, $20.00 to Tier 2",
  "SUITE AD"
);

const tierHigh = calculateCartTierDiscount(12000);
assert(
  tierHigh.discount === 2400 && tierHigh.total === 9600 && tierHigh.nextTierThreshold === 0,
  "AD1.3: $120.00 subtotal receives 20% ($24.00) discount, total = $96.00 (Max Tier reached)",
  "SUITE AD"
);
