...
if (violations.length === 0) {
  try {
    const scannerOutput = execSync(`node ${path.join(__dirname, 'static-analysis.cjs')} --fix-report`, {
      encoding: 'utf8',
      cwd: ROOT,
  ...
  switch (rule) {
    case 'button-disabled-finally-guarantee':
      return {
        ...
        rationale: 'Button is disabled to show a loading spinner on submission, but lacks a .finally() or catch re-enabling statement.',
    ...
    case 'add-to-cart-response-status-check':
      return {
        category: '🛒 Add-to-Cart / Checkout Blocker',
        rationale: 'The fetch() call to /cart/add.js parses JSON without checking response.ok or response.status === 200.',
        impact: 'When an item is sold out or restricted, Shopify returns a 422 error payload. The script assumes success, triggers drawer animations, but the cart remains empty. Shoppers proceed to checkout only to discover zero items.'
      };
    case 'no-unsafe-inline-onclick':
      return {
        category: '📦 High-AOV Bundle & Box Builder Blocker',
        rationale: 'Dynamic single-quoted template literals in onclick attributes (e.g. selectBrand(\'${brand}\')) fail when string values contain apostrophes.',
    ...
    case 'form-variant-id-completeness':
      return {
        ...
        impact: 'Shopify backend rejects form submission with HTTP 400 Bad Request, preventing the product from ever reaching the cart.'
      };
    case 'cart-drawer-sync-integrity':
      return {
        category: '🛍️ Cart Drawer & UI Sync Warning',
        rationale: 'Event listeners attempt to trigger defunct Dawn <cart-drawer> elements instead of Scentspired\'s custom #sp-cart-drawer and updateDossierCartUI().',
        impact: 'Items add in the background but the cart drawer never opens. Customers click the Add to Cart button repeatedly, resulting in accidental duplicate items or frustration.'
      };
    case 'balanced-script-tags':
      return {
        ...
        impact: 'Breaks HTML DOM parsing entirely, causing subsequent elements on the page to fail rendering and terminating all client-side scripts.'
      };
    ...
    case 'no-unguarded-dom-variable':
      if (file.includes('header')) {
        return {
          ...
          rationale: 'DOM element query accessed without null verification on viewports where the element is unrendered.',
          impact: 'Uncaught TypeError in the global header halts all downstream JavaScript execution on the entire page, killing sticky buy buttons, variant pickers, and currency switchers.'
        };
      }
      if (file.includes('cart') || file.includes('bundle') || file.includes('product-form') || file.includes('quick-add') || file.includes('best-sellers') || file.includes('discovery') || file.includes('trio-set') || file.includes('five-box')) {
        return {
          category: '🛒 Purchase Funnel DOM Lookup Failure',
          rationale: 'Crucial form elements (variant IDs, quantity selectors, pricing nodes) accessed without checking for null return.',
          impact: 'Throws TypeError when interacting with variants, causing buttons to become unresponsive and preventing line items from being formatted for cart addition.'
        };
      }
      return {
        category: '⚡ Secondary UI & Visual Element Crash',
        rationale: 'DOM node lookup accessed directly without null-checking.',
        impact: 'Can throw runtime exceptions that clutter browser console and may interrupt smooth page rendering or interactive tabs.'
      };
    case 'fetch-must-have-catch':
      return {
        ...
        impact: 'Transient network disconnects cause unhandled promise rejections, leaving UI loading states hanging indefinitely.'
      };
    case 'no-unguarded-closest-chain':
      return {
        ...
        impact: 'Throws runtime TypeError if user clicks on modified HTML structure or detached elements.'
      };
    case 'checkout-url-integrity':
      return {
        category: '💳 Checkout Gateway Redirection Blocker',
        rationale: 'Checkout trigger button does not validate active cart state or checkout endpoint.',
...
const purchaseFiles = [
  'product-form.js', 'quick-add.js', 'cart.js', 'cart-drawer.liquid', 'cart-drawer.js',
  'header.liquid', 'five-box.liquid', 'trio-set.liquid', 'bundle.liquid', 'discovery.liquid',
  'best-sellers.liquid', 'bundlediscovery.liquid', 'main-cart-items.liquid', 'main-cart-footer.liquid',
  'predictive-search.js', 'theme.liquid', 'Video-banner1.liquid', 'mobile-video-banner.liquid'
];

const purchaseViolations = violations.filter(v => 
  purchaseFiles.some(f => v.file.includes(f)) || 
  ['button-disabled-finally-guarantee', 'add-to-cart-response-status-check', 'no-unsafe-inline-onclick', 'checkout-url-integrity', 'form-variant-id-completeness'].includes(v.rule)
);
...
md += "```\n";
md += "┌────────────────────────────────────────────────────────────────────────────────────────┐\n";
md += "│                          PURCHASE-PATH FAILURE MATRIX                                  │\n";
md += "├────────────────────────────────────────────────────────────────────────────────────────┤\n";
md += "│ 🔴 Frozen \"Adding...\" Buttons (Missing .finally() Re-enabling)     │   5 Defect Sites  │\n";
...
console.log(`   → Total Defects Cataloged: ${violations.length} items across ${Object.keys(byFile).length} files.`);
