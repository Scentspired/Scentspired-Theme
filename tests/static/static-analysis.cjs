...
function isApproved(filePath, lineNum, ruleName) {
  ...
  return config.approvedExceptions.some(
    ex =>
...
const RULE_SCANNERS = {
  ...
  "no-unguarded-getElementById": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (directAccessPattern.test(line)) {
        ...
        const isGuarded =
          (prevLine.includes(`getElementById('${elementId}')`) &&
            (prevLine.startsWith("if") ||
              prevLine.startsWith("const") ||
              prevLine.startsWith("let") ||
              prevLine.startsWith("var"))) ||
          (prevPrevLine.includes(`getElementById('${elementId}')`) &&
            (prevPrevLine.startsWith("if") ||
              prevPrevLine.startsWith("const") ||
              prevPrevLine.startsWith("let") ||
              prevPrevLine.startsWith("var")));
  ...
  "no-unguarded-querySelector": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (directAccessPattern.test(line)) {
        ...
        const isGuardedByContext =
          context.includes(`querySelector('${selector}')`) &&
  ...
  "no-unsafe-inline-onclick": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (unsafePattern.test(line)) {
        ...
        if (
          /^\d+$/.test(varName) ||
          (/^[a-z_]+$/i.test(varName) && ["i", "j", "k", "index", "idx"].includes(varName))
        ) {
          continue;
        }
  ...
  "no-unguarded-closest-chain": (content, filePath, lines) => {
    ...
    const directAccessPattern =
      /\.closest\s*\([^)]+\)\s*\.\s*(querySelector|querySelectorAll|classList|style|value|textContent|innerHTML|innerText|disabled|setAttribute|getAttribute|removeAttribute|children|firstChild|addEventListener)/;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (directAccessPattern.test(line)) {
        ...
        const isGuarded =
          prevLines.includes(`closest('${selector}')`) &&
  ...
  "no-unguarded-getElementById": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (matchesDirect || matchesVar) {
        ...
        const context = lines.slice(Math.max(0, i - 3), i).join(" ");
        const isGuarded =
          (context.includes(`getElementById`) &&
            (context.includes("if (") || context.includes("if(") || context.includes("if (!"))) ||
          (prevLine.includes(`getElementById('${elementId}')`) &&
            (prevLine.startsWith("if") ||
              prevLine.startsWith("const") ||
              prevLine.startsWith("let") ||
  ...
  "add-to-cart-response-status-check": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        (line.includes("/cart/add.js") || line.includes("/cart/add")) &&
        !line.includes("<form") &&
  ...
  "cart-drawer-sync-integrity": (content, filePath, lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        line.includes("querySelector('cart-drawer')") ||
        line.includes('querySelector("cart-drawer")')
      ) {
        ...
        const hasModernFallback =
          context.includes("updateDossierCartUI") ||
          context.includes("openDossierCart") ||
          context.includes("#sp-cart-drawer") ||
          content.includes("updateDossierCartUI");
  ...
  "no-missing-snippet-assets": function (content, filePath, lines) {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      if (
        snippetName.includes("{{") ||
        snippetName.includes(" ") ||
        snippetName.includes("|") ||
        snippetName === "cart-drawer-complete"
      )
        continue;
  ...
  "bundle-inventory-availability-guard": function (content, filePath, lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes("function addProductToBundle(")) {
        ...
        const hasAvailableGuard =
          fnBlock.includes("available === false") ||
          fnBlock.includes("!variant.available") ||
          fnBlock.includes("available !== false");
        if (
          !hasAvailableGuard &&
          !isApproved(filePath, i + 1, "bundle-inventory-availability-guard")
        ) {
          violations.push(
            violation(
              filePath,
              i + 1,
              "bundle-inventory-availability-guard",
  ...
  "no-swallowed-cart-error-message": function (content, filePath, lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes("catch (err)") || line.includes("catch(err)")) {
        ...
        if (
          catchBlock.includes("alert('Error") ||
          catchBlock.includes('alert("Error') ||
          catchBlock.includes("alert('Failed") ||
          catchBlock.includes('alert("Failed')
        ) {
          const usesErrMessage =
            catchBlock.includes("err.message") ||
            catchBlock.includes("errorData") ||
            catchBlock.includes("err");
          if (!usesErrMessage && !isApproved(filePath, i + 1, "no-swallowed-cart-error-message")) {
            violations.push(
              violation(
                filePath,
                i + 1,
                "no-swallowed-cart-error-message",
...
function runScan() {
  ...
  for (const filePath of files) {
    ...
    for (const [ruleName, scanner] of Object.entries(RULE_SCANNERS)) {
      ...
      if (
        onlyRule &&
        !ruleName.includes(onlyRule) &&
        !ruleName.endsWith(`-${onlyRule}`) &&
        ruleName !== `rule-${onlyRule}`
      )
        continue;
      ...
      try {
        ...
        if (!fixReport) {
          console.error(
            `  [SCANNER ERROR] Rule "${ruleName}" crashed on ${path.relative(ROOT, filePath)}: ${e.message}`
          );
        }
      }
    }
  ...
  if (allViolations.length === 0) {
    ...
    console.log(`  Scanned ${filesScanned} files (${linesScanned.toLocaleString()} lines)`);
    console.log(
      `  Found ${allViolations.length} violations (${errorCount} errors, ${warnCount} warnings)`
    );
    console.log("");
  ...
  console.log("┌──────────────────────────────────────────────────────────────┐");
  console.log(
    `│  Files: ${String(filesScanned).padStart(4)}  │  Lines: ${String(linesScanned.toLocaleString()).padStart(7)}  │  Rules: ${String(Object.keys(RULE_SCANNERS).length).padStart(2)}  │  Exceptions: ${String(config.approvedExceptions.length).padStart(2)} │`
  );
  console.log(
    `│  Errors: ${String(errorCount).padStart(3)}  │  Warnings: ${String(warnCount).padStart(4)}                                │`
  );
  console.log("└──────────────────────────────────────────────────────────────┘");
